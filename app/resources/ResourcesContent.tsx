...
import { electedOfficials, resourceCategories } from "@/data/resources";
...
export default function ResourcesContent() {
  ...
  const filteredCategories = useMemo(() => {
    if (!normalizedQuery) return resourceCategories;

    return resourceCategories
      .map((cat) => {
        const matchesCategory = normalizeText(cat.title).includes(normalizedQuery);
        const resources = matchesCategory
          ? cat.resources
          : cat.resources.filter((resource) => {
              const haystack = `${resource.name} ${resource.description}`;
  ...
  const navCategories = normalizedQuery ? filteredCategories : resourceCategories;
  ...
  return (
    <>
      ...
      <nav className="bg-white border-b border-gray-200 sticky top-0 z-30 shadow-sm" aria-label="Jump to resource category">
        <div
          className="max-w-6xl mx-auto px-4 py-3 overflow-x-auto"
      ...
      <div className="max-w-6xl mx-auto px-4 py-12 md:py-16 space-y-16">
        <div className="max-w-xl">
          ...
          <input
            ...
            placeholder="Search by organization or service"
            className="w-full rounded-xl border border-gray-300 bg-white px-4 py-3 text-sm text-text placeholder:text-text/40 focus:outline-none focus:ring-2 focus:ring-skyblue/70 focus:border-skyblue/80"
          />
        </div>

        {filteredCategories.length === 0 ? (
          <div className="rounded-xl border border-gray-200 bg-light-gray p-6">
            <p className="text-text/70">
              No matches found for <span className="font-semibold text-navy">{query}</span>. Try a broader term like
              {" "}housing, food, health, or jobs.
            </p>
          </div>
        ) : (
        ...
        <section id="officials" aria-labelledby="officials-heading">
          ...
          <div className="mb-8 max-w-2xl rounded-xl bg-navy/5 border border-navy/20 p-4">
            <p className="text-text/70">
      ...
      <div className="space-y-1 mt-auto pt-3 border-t border-gray-100">
        {resource.phone && (
          <p className="text-sm">
            ...
            <a href={toPhoneHref(resource.phone)} className="text-navy font-medium hover:underline">
              {resource.phone}
            </a>
          </p>
        )}
...
function OfficialCard({ official }: { official: import("@/data/resources").ElectedOfficial }) {
  const partyColor =
    official.party === "D" ? "bg-blue-100 text-blue-700"
      ...
      : official.party === "L" ? "bg-yellow-100 text-yellow-700"
    ...
    <div className="bg-white rounded-xl border border-gray-200 p-5 shadow-sm">
      ...
      <div className="space-y-1">
        {official.phone && (
          <p className="text-sm">
            <span className="text-text/40 mr-1" aria-hidden="true">📞</span>
            <a href={toPhoneHref(official.phone)} className="text-navy font-medium hover:underline">
              {official.phone}
            </a>
          </p>
        )}
